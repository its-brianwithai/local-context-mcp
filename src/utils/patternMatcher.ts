...
export async function searchFilesByRegex(files: string[], patterns: string[]): Promise<string[]> {
  if (!files || files.length === 0 || !patterns || patterns.length === 0) {
    return [];
  }
  ...
  for (const pattern of patterns) {
    try {
      ...
      console.warn(`Invalid regex pattern "${pattern}": ${(error as Error).message}`);
    }
  }
    ...
    await Promise.all(chunk.map(async (filePath) => {
      try {
        ...
        if (err.code !== 'ENOENT' && err.code !== 'EISDIR') {
          console.debug(`Could not read file ${filePath}: ${err.message}`);
        }
      }
    }));
  }
...
export async function findMatchingFiles(
  ...
  if ((!globs || globs.length === 0) && (!regex || regex.length === 0)) {
    return [];
  }
