...
async function extractImports(filePath: string): Promise<string[]> {
  try {
    ...
    console.debug(`Could not read imports from ${filePath}:`, error);
    return [];
  }
}
 ...
 */
async function resolveImportPath(importPath: string, currentFile: string, projectRoot: string): Promise<string | null> {
  ...
  while (maxDepth === -1 || currentDepth <= maxDepth) {
    ...
    for (const file of filesToSearch) {
      ...
      for (const importPath of imports) {
        ...
        if (resolvedPath && references.has(resolvedPath)) {
...
function isSourceFile(filename: string): boolean {
  const sourceExtensions = ['.dart', '.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.kt', '.swift', '.go', '.rs', '.rb', '.php', '.cpp', '.c', '.h', '.cs'];
...
async function findAllSourceFiles(dir: string): Promise<string[]> {
  ...
  async function walk(currentDir: string): Promise<void> {
    try {
      ...
      console.debug(`Could not read directory ${currentDir}:`, error);
    }
  }
