...
function extractCompleteSignature(lines: string[], startIndex: number): string {
  ...
  while ((openBraces > 0 || openParens > 0 || openBrackets > 0 || inString || 
          signature.trim().endsWith(',') || signature.trim().endsWith('=>')) && 
    ...
    if (openBraces === 0 && openParens === 0 && openBrackets === 0 && !inString && 
...
function extractClasses(content: string): ExtractedCode['classes'] {
  ...
  const methodPattern = /^\s*(static\s+|final\s+|const\s+|late\s+)*([\w<>\[\]?]+\s+)?(get\s+|set\s+)?(\w+)\s*\(/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch) {
      ...
      while (j < lines.length && braceCount > 0) {
        ...
        const constructorMatch = trimmedMethodLine.match(new RegExp(`^${className}\\s*\\(`)) ||
...
function removeClassBodies(lines: string[]): string[] {
  ...
  for (let i = 0; i < cleanedLines.length; i++) {
    ...
    if (!inClass && (trimmedLine.includes('class ') || 
                    trimmedLine.includes('enum ') || 
                    trimmedLine.includes('mixin ') ||
